//src/app/api/customer/orders/[id]/invoice/route.ts - DISESUAIKAN DENGAN FORMAT FINANCE
...
export async function GET(
  ...
  try {
    ...
    // Get order dengan payment yang sudah PAID
    ...
    if (!order || order.payments.length === 0) {
      return NextResponse.json(
        { error: "Invoice tidak tersedia atau pembayaran belum lunas" },
        { status: 404 }
      );
    }
    ...
    const headerHeight = 45; // Sama seperti finance
    ...
    const separatorY = margin + headerHeight - 8; // Dinaikkan seperti finance
    ...
    // Tanggal faktur = tanggal pembayaran (jika ada) atau tanggal payment dibuat
    ...
    // Jatuh tempo = H+1 setelah order dibuat
    const orderDate = new Date(order.createdAt);
    const dueDate = new Date(orderDate);
    dueDate.setDate(dueDate.getDate() + 1); // H+1
    ...
    doc.text(`Up ${order.user.name}`, margin, customerInfoY + 6);
    ...
    doc.text(`${order.orderNumber}/HMI/IX/${paymentDate.getFullYear()}`, margin + 25, infoY + 6);
    ...
    const rowHeight = 10; // ✅ KONSISTEN DENGAN FINANCE
    const headerHeight_table = 12; // ✅ KONSISTEN DENGAN FINANCE
    ...
    // ✅ STATUS - RIGHT ALIGNED AT BOTTOM (SELALU LUNAS UNTUK CUSTOMER)
    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0, 150, 0); // Hijau untuk LUNAS
    const statusWidth = doc.getTextWidth('Status: LUNAS');
    doc.text('Status: LUNAS', pageWidth - margin - statusWidth, footerY);
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Faktur-${order.orderNumber}.pdf"`
      }
    });
