// src/app/admin/orders/create/page.tsx - DENGAN PAYMENT METHOD
"use client";
...
// Interfaces yang sama seperti sebelumnya...
interface User {
  id: number;
  name: string;
  email: string;
...
export default function CreateOrderPage() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch('/api/users?role=CUSTOMER&limit=50');
      if (response.ok) {
  ...
  try {
    console.log("🔍 Fetching services for order...");
    ...
    console.log("Services API response ok:", response.ok);
    ...
    toast.error(`Gagal memuat jasa: ${error.message}`);
    setServices([]);
  }
};

  // Functions untuk manage order items sama seperti sebelumnya...
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Customer Selection - sama seperti sebelumnya */}
          ...
          {/* Product/Service Selection - sama seperti sebelumnya tapi lebih compact */}
          <div className="lg:col-span-2">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-80 overflow-y-auto">
                {/* Products & Services content sama seperti sebelumnya */}
        ...
        {/* Order Items - sama seperti sebelumnya */}
        {orderItems.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold mb-4 flex items-center">
            ...
            <div className="space-y-3">
              {orderItems.map((item, index) => (
                <div key={`${item.type}-${item.id}`} className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-md">
        ...
        {/* Additional Info dengan Payment Method */}
            ...
            {paymentMethod && (
              <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md">
                <div className="flex items-center text-blue-600 dark:text-blue-400">
                  ...
                  <span className="font-medium">Metode Terpilih:</span>
                </div>
        ...
        <div className="flex justify-end space-x-4 pt-6">
          ...
          <button
            type="submit"
            disabled={isLoading || !selectedUser || orderItems.length === 0 || !paymentMethod}
